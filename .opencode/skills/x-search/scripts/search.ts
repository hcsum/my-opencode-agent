...
async function searchX(input: SearchInput): Promise<ScriptResult> {
  ...
  try {
    targetId = await openBackgroundTab(searchUrl);
    ...
    if (blocked) {
      return { success: false, message: "X showed a block or challenge page." };
    }
    ...
    for (let step = 0; step < 10 && items.length < limit; step += 1) {
      const batch = await evalInTab<SearchResult[]>(
        targetId,
        `(() => {
          const out = [];
    ...
    if (items.length === 0) {
      return {
        success: false,
        message: `No results found for "${input.query}". X may be blocking or the query returned nothing.`,
      };
    }
